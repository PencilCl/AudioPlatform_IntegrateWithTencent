        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (Environment.getExternalStorageState()// 如果外存存在
                    ...
                    audioName=AppCondition.getAppExternalDir()+File.separator+subDir+File.separator+getRecordedFileName("_Watch"+(fileNumber+1));//命名方式有些奇怪，后续要改进

                }
                else{//否则
                    Log.i(TAG,"mark2: the device has not got a external storage");
                    audioName=subDir+File.separator+getRecordedFileName("_Watch"+(fileNumber+1));//命名方式有些奇怪，后续要改进
                }
